...
public class GUI extends JPanel implements LayoutCompleteEventListener {
    ...
    public static final JMenu TOOL_MENU = new JMenu("Tools");
    /**
     * for serialization
     */
    ...
    class Selection {
        ...
        public void draw(Graphics g) {
            ...
            for (int iNode = 0; iNode < m_selected.size(); iNode++) {
                ...
                int nYRC = nPosY;
                int d = 5;
                System.out.println("esto es g" + g);
                    ...
                    public void actionPerformed(ActionEvent ae) {
                        ...
                        try {
                            ...
                            if (m_marginCalculator == null) {
                                ...
                                System.out.println("--m_marginCalculator-->" + m_marginCalculator);
                            }
                            text.append(m_marginCalculator.toXMLBIF03());
                            System.out.println("m_marginCalculator.toXMLBIF03--> " + text);
    ...
    private void makeMenuBar() {
        ...
        JMenu TOOL_MENU = new JMenu("Tools");
        TOOL_MENU.setMnemonic('T');
        TOOL_MENU.add(a_networkgenerator);
        TOOL_MENU.add(a_datagenerator);
        TOOL_MENU.add(a_datasetter);
        TOOL_MENU.add(a_learn);
        TOOL_MENU.add(a_learnCPT);
        TOOL_MENU.addSeparator();
        TOOL_MENU.add(a_layout);
        TOOL_MENU.addSeparator();
        System.out.println("tool_menu" + a_networkgenerator);
        final JCheckBoxMenuItem viewMargins = new JCheckBoxMenuItem("Show Margins",
                false);
        System.out.println("viewMargins-->" + viewMargins.getMargin());
        ...
        viewMargins.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                ...
                System.out.println("-- bprev-->" + bPrev + viewMargins.getAlignmentX() + " :O " + viewMargins.getIconTextGap() + " " + m_bViewMargins);
                if (bPrev == false && viewMargins.getState() == true) {
                    updateStatus();
                    System.out.println("-viewMargins->" + viewMargins.getState() + "-ojo->" + viewMargins.getMargin().toString() + " " + m_bViewMargins);
                    System.out.println(" get all viewMargins--> " + viewMargins.getX() + viewMargins.getComponentZOrder(viewMargins) + viewMargins.getText() + viewMargins.getActionCommand());
                    System.out.println("getAccessibleContext--> " + viewMargins.getAccessibleContext().getAccessibleDescription() + "++++" + viewMargins.getBaselineResizeBehavior().toString() + "+++" + viewMargins.getX());
                    System.out.println("getIconTextGap()-->1" + viewMargins.getIconTextGap() + "--" + viewMargins.getAlignmentX() + viewMargins.getBaselineResizeBehavior());
                }
                repaint();
            }
        });
        System.out.println("tool menu " + TOOL_MENU.toString());
        TOOL_MENU.add(viewMargins);
        System.out.println("-m_bViewMargin3s->" + m_bViewMargins + "viewMargins2148" + viewMargins);
        ...
        viewCliques.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent ae) {
                ...
                System.out.println("-m_bViewCliques->" + bPrev);
        ...
        TOOL_MENU.add(viewCliques);

        m_menuBar.add(TOOL_MENU);
    ...
    private class GraphPanel extends PrintablePanel implements Printable {
        ...
        public void paintComponent(Graphics gr) {
            ...
            System.out.println("--m_fScale--" + m_fScale + "--" + m_fScale);
            ...
            for (int iNode = 0; iNode < m_BayesNet.getNrOfNodes(); iNode++) {
                ...
                System.out.println("g+draw-->" + g + "g+draw" + NORMAL + iNode);
        ...
        protected <DistributionEstimator> void drawNode(Graphics g, int iNode, int mode) {
            ...
            if (fm.stringWidth(m_BayesNet.getNodeName(iNode)) <= m_nNodeWidth) {
                ...
                System.out.println("m_BayesNet-->" + fm + "-bays-" + m_BayesNet + "paddN--" + m_nPaddedNodeWidth);

                System.out.println("m_BayesNet.getNodeName-->" + m_BayesNet.getNodeName(iNode));
                System.out.println(fm.stringWidth(m_BayesNet.getNodeName(iNode)) + "-posy-" + nPosY);
                System.out.println("m_nPaddedNodeWidth--> m_nPadd" + m_nPaddedNodeWidth);
            ...
            if (mode == HIGHLIGHTED) {
                ...
                System.out.println("g setXORMode--->" + g.toString());
            }
...
///////////////////////////////////////
        ...
        private double[] printMarginWithNoParents(int iNodeP, int iValue) {
            ...
            if (!hasNoParents(iNodeP)) {
                ...
                double pFinalPar = 0;
                double[] marginParentsX = new double[0];
                double[] marginParentsY;
                ...
                if (hasNoParents) {
                    ...
                    for (int parentIndex : parentSet) {
                        ...
                        marginParentsX=marginParents;
                    }

                    double[] margin = getMargin(iNodeP, iValue);
                    System.out.println(" probabilitie margin-->" + margin[iValue]);
                    marginParentsY=margin;

                    System.out.println("segundo nivel......."+marginParentsX[iValue]+marginParentsY[iValue] );
                    // Suponiendo que marginParentsX y marginParentsY tienen la misma longitud
                    double[] resultado = new double[marginParentsX.length];

                    for (int i = 0; i < marginParentsX.length; i++) {
                        resultado[i] = marginParentsX[i] * marginParentsY[i];
                        pFinal=resultado;
                ...
                if (!hasNoParents) {
                 ...
                 //   System.out.println("Tercer nivel......!"+marginParentsW[iValue]+marginParentsY[iValue] );
        ...
        public void posteriorProbability(int iNode) {
            ...
            try {
                ...
                // Set values for the test instance using parent instances
                for (int parentIndex : parentIndices) {
                    Instance parentInstance = instances.get(parentIndex);
                    for (int j = 1; j < parentInstance.numAttributes(); j++) { // Exclude the class attribute
                        double parentValue = parentInstance.value(j);
                        Attribute parentAttribute = parentInstance.attribute(j);
    ...
    private class GraphVisualizerMouseListener extends MouseAdapter {
        ...
        void handleRightNodeClick(MouseEvent me) {
            ...
            try {
                ...
                System.out.println("-m_bViewMargins2->" + m_bViewMargins);
                ...
                popupMenu.setLocation(me.getX(), me.getY());
                popupMenu.show(m_GraphPanel, me.getX(), me.getY());
                System.out.println("-- m_GraphPanel-g" + m_GraphPanel);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    void updateStatus() {
        ...
        System.out.println(" updateStatus->" + a_undo);
        ...
        if (!m_bViewMargins && !m_bViewCliques) {
            System.out.println("-m_bViewMargins update->" + m_bViewMargins);
            repaint();
            return;
        }
    ...
    void deleteArc(String[] options) {
        ...
        if (sResult != null && !sResult.equals("")) {
            ...
            System.out.println("--Parent and Child" + sParent + sChild);
            try {
    ...
    public static void main(String[] args) {
        ...
        JFrame jf = new JFrame("VARIAMOS WEKA");
        final GUI g = new GUI();
